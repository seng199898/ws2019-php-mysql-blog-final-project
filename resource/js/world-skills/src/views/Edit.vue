<template>
  <div class="create">
  	...
  	<form class="container" action="#" method="get" @submit.prevent="submit" v-if="post.title">
  		<input type="text" name="title" placeholder="Title" :value="post.title">
  		<textarea name="content" placeholder="Type some post content here." >{{ post.content }}</textarea>

  		<select name="category" v-model="post.category.id">
...
export default {
  ...
  created() {
	fetch('/api/post?edit=1&slug=' + this.$route.params.id).then(res => res.json()).then((json) => {
		if(json.success) {
			this.post = json.post;
		} else {
			swal({
  ...
  methods: {
  	submit(e) {
  		...
  		if (title == '' || content == '' || category == '') {
  			...
  			fetch('/api/update?slug=' + this.$route.params.id, {
  				method: 'post',
  				body: this.formData({
  					title,
  					content,
